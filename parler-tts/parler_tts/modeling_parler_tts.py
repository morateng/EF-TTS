...
from .peft_modules import PrecomputedVectorPEFT
...
class ParlerTTSForConditionalGeneration(PreTrainedModel):
    ...
    def __init__(
        ...
        # Initialize PEFT module for precomputed vectors
        self.vector_peft = None
        if config.use_precomputed_vectors:
            self.vector_peft = PrecomputedVectorPEFT(
                vector_dim=config.precomputed_vector_dim,
                num_vectors=config.num_precomputed_vectors,
                attribute_values=config.attribute_values,
                lora_rank=config.lora_rank,
                lora_alpha=config.lora_alpha,
                vae_latent_dim=config.vae_latent_dim,
                orthogonal_reg_strength=config.orthogonal_reg_strength
            )
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        loss_reduction: str = "mean",
        precomputed_vectors: Optional[torch.FloatTensor] = None,
        description_tokens: Optional[List[str]] = None,
        vae_loss_weight: float = 0.5,
        orthogonality_loss_weight: float = 1.0,
        **kwargs,
        ...
        if encoder_outputs is None:
            # Use precomputed vectors if provided and enabled in config
            if precomputed_vectors is not None and self.config.use_precomputed_vectors:
                # Apply PEFT transformations to precomputed vectors
                if self.vector_peft is not None:
                    ...
                    encoder_hidden_states = peft_results["enhanced_vectors"]
                    
                    # Store PEFT losses for later integration
                    if self.training and "vae_loss" in peft_results:
                        self._peft_vae_loss = peft_results["vae_loss"]
                        self._peft_orthogonality_loss = peft_results["orthogonality_loss"]
                else:
                    encoder_hidden_states = precomputed_vectors

                # optionally project precomputed vectors
                if (
                    self.config.precomputed_vector_dim != self.decoder.config.hidden_size
                    and self.decoder.config.cross_attention_hidden_size is None
                ):
                    encoder_hidden_states = self.enc_to_dec_proj(encoder_hidden_states)

                # Create encoder_outputs compatible with existing code
                from transformers.modeling_outputs import BaseModelOutput
                encoder_outputs = BaseModelOutput(last_hidden_state=encoder_hidden_states)
            else:
                # Use text encoder as before
        ...
        # Integrate PEFT losses if available
        total_loss = decoder_outputs.loss
        if total_loss is not None and hasattr(self, '_peft_vae_loss'):
            # Add PEFT losses with appropriate weighting
            total_loss = total_loss + (vae_loss_weight * self._peft_vae_loss) + (orthogonality_loss_weight * self._peft_orthogonality_loss)
            
            # Clean up stored losses
            delattr(self, '_peft_vae_loss')
            delattr(self, '_peft_orthogonality_loss')
    ...
    def _prepare_text_encoder_kwargs_for_generation(
        ...
        # Check if using precomputed vectors
        if "precomputed_vectors" in model_kwargs and self.config.use_precomputed_vectors:
            ...
            if self.vector_peft is not None:
                ...
                peft_results = self.vector_peft(
                    precomputed_vectors,
                    description_tokens=description_tokens, 
                    return_losses=False  # No losses during generation
                )
                encoder_hidden_states = peft_results["enhanced_vectors"]
            else:
                encoder_hidden_states = precomputed_vectors
