...
class ModelArguments:
    ...
    use_precomputed_vectors: bool = field(
        default=False,
        metadata={"help": "Whether to use precomputed vectors instead of text encoder"}
    )
    vector_base_path: str = field(
        default="./vectors",
        metadata={"help": "Path to precomputed vector files"}
    )
    vae_loss_weight: float = field(
        default=0.5,
        metadata={"help": "Weight for VAE loss in PEFT training"}
    )
    orthogonality_loss_weight: float = field(
        default=1.0,
        metadata={"help": "Weight for orthogonality loss in PEFT training"}
    )
...
class DataTrainingArguments:
    ...
    rebuilt_caption_column_name: str = field(
        default="rebuilt_caption",
        metadata={"help": "The name of the dataset column containing the style caption data for vector-based training."},
    )
