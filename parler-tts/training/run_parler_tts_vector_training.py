...
from training.data import load_multiple_datasets, DataCollatorParlerTTSWithPadding, DataCollatorEncodecWithPadding, DataCollatorParlerTTSWithVectors
...
def main():
    ...
    if dataset_was_precomputed:
        ...
        # For vector training, load dataset differently
        if model_args.use_precomputed_vectors:
            ...
            if training_args.do_train:
                ...
                required_columns = [
                    ...
                    data_args.prompt_column_name,
                    data_args.rebuilt_caption_column_name
                ]
                
                # Filter out columns that don't exist
                available_columns = raw_datasets["train"].column_names
                ...
                raw_datasets["train"] = raw_datasets["train"].select_columns(columns_to_keep)
            ...
            if training_args.do_eval:
                ...
                # Keep only required columns for eval
                raw_datasets["eval"] = raw_datasets["eval"].select_columns(columns_to_keep)
                
                if data_args.max_eval_samples is not None:
                    raw_datasets["eval"] = raw_datasets["eval"].select(range(data_args.max_eval_samples))
                
        else:
            # Original data loading for non-vector training
            columns_to_keep = {
            ...
            if training_args.do_eval:
                raw_datasets["eval"] = load_multiple_datasets(
                    ...
                    if data_args.eval_metadata_dataset_name
                    else data_args.train_metadata_dataset_name,
    ...
    # Initialize vector loader for vector training
    vector_loader = None
    if model_args.use_precomputed_vectors:
        from parler_tts.vector_utils import VectorLoader
    ...
    if not dataset_was_precomputed and not model_args.use_precomputed_vectors:
    ...
    elif model_args.use_precomputed_vectors:
        # For vector training, we use raw_datasets directly as vectorized_datasets
        vectorized_datasets = raw_datasets
        logger.info("Using precomputed vectors - skipping audio encoding")
    ...
    if padding == "max_length" and not model_args.use_precomputed_vectors:
    ...
    def train_step(
        ...
        # For vector training, precomputed_vectors are already in the batch
        if not model_args.use_precomputed_vectors:
        ...
        outputs = model(
            **batch, 
            ...
            orthogonality_loss_weight=model_args.orthogonality_loss_weight
        )
        # CE (data) loss
