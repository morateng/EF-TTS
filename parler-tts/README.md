...
* [Usage](#usage)
  ...
  - [🚀 Using precomputed vectors (PEFT enhanced)](#-using-precomputed-vectors-peft-enhanced)
* [Training](#training)
...
### 🚀 Using Precomputed Vectors (PEFT Enhanced)

For faster inference and more precise style control, you can use models trained with precomputed vectors and PEFT (Parameter-Efficient Fine-Tuning). This approach skips the text encoder at runtime for improved efficiency:
...
# Load a model configured for precomputed vectors
model = ParlerTTSForConditionalGeneration.from_pretrained("your-peft-model").to(device)
tokenizer = AutoTokenizer.from_pretrained("parler-tts/parler-tts-mini-v1")

# Prepare precomputed style vectors (shape: [batch_size, num_vectors, vector_dim])
# Example: 8 vectors of 1024 dimensions each representing different style attributes
precomputed_vectors = torch.randn(1, 8, 1024).to(device)

# You can also compose specific styles by selecting and combining vectors:
# female_voice = precomputed_vectors[0, 0]     # Gender: female
# high_pitch = precomputed_vectors[0, 1]       # Pitch: high  
# clear_audio = precomputed_vectors[0, 2]      # Quality: clear
# american_accent = precomputed_vectors[0, 3]  # Accent: American
# moderate_speed = precomputed_vectors[0, 4]   # Speed: moderate
# ... (additional style vectors)
...
audio_arr = generation.cpu().numpy().squeeze()
sf.write("parler_tts_peft_out.wav", audio_arr, model.config.sampling_rate)
```

**Benefits of the PEFT approach**:
* **Faster inference**: Bypasses text encoding at runtime
* **Precise control**: Each vector can represent specific style attributes (gender, pitch, accent, etc.)
* **Composable styles**: Mix and match different style vectors for custom voice characteristics
* **Parameter efficiency**: Only PEFT parameters are trained, keeping the base model frozen
